...
뚜껑을 닫은 맥은 한 번에 수십 초씩만 깨어납니다. runner 는 그 안에 끝나지만 `$COMMAND` 는 대개 그렇지 않아서, 도중에 얼었다가 다음 wake 에 이어지고 붙들고 있던 연결은 끊깁니다.

그래서 `run.sh` 가 작업이 도는 동안 커널의 `SleepDisabled` 플래그를 걸고, 작업이 없으면 풉니다. `caffeinate` 로는 안 됩니다 — 뚜껑 닫힘을 넘는 건 이 플래그뿐입니다.

```bash
pmset -g | grep SleepDisabled   # 작업 중이면 1, 아니면 0
```

플래그는 매 사이클 다시 확인하므로 작업이 풀지 못하고 죽어도 최대 한 사이클이면 해제됩니다. 다만 이 플래그는 재부팅을 넘어 유지되고, 걸려 있는 동안 맥은 전혀 잠들지 않습니다 — 배터리에서는 그만큼 소모됩니다.
