...
뚜껑을 닫고 배터리로 있으면 `pmset` wake 가 맥을 완전히 깨우지 못합니다. **dark wake** 로만 올라오는데, 이건 수십 초짜리 유지보수 창이고 그 뒤엔 곧바로 다시 잠듭니다. runner 는 그 안에 끝나지만 `$COMMAND` 는 대개 그렇지 않습니다 — 도중에 동결됐다가 다음 wake 에 해동되므로, CPU 몇 분이면 되는 작업이 한 시간에 걸쳐 흩어지고, 동결 순간 붙들고 있던 연결은 해동 후 이미 끊겨 있습니다.

**어떤 파워 어시션으로도 해결되지 않습니다.** `caffeinate -i` 가 거는 것은 `PreventUserIdleSystemSleep` 인데, 뚜껑을 닫으면 `Clamshell Sleep` 이 걸리고 dark wake 는 `Maintenance Sleep` 으로 끝납니다. 둘 다 idle sleep 이 아니므로 어시션은 정상적으로 유지되지만 애초에 해당 사항이 없습니다. `-s` 는 배터리에서 무시됩니다. 리드 스위치는 `IOPMrootDomain` 으로 직접 들어가는 하드웨어 경로라 어떤 어시션도 이를 넘지 못합니다.

이를 넘어서는 것은 커널의 `SleepDisabled` 플래그뿐입니다. 그래서 `run.sh` 가 **작업이 있는 동안만** 이 플래그를 잡습니다.
...
판단은 매 사이클마다 실제 프로세스 목록을 보고 다시 내립니다. 그래서 이 구조가 곧 정리 경로이기도 합니다 — 작업이 플래그를 풀지 못하고 죽어도 최대 한 사이클이면 해제됩니다. `SleepDisabled` 는 **재부팅을 넘어 유지**되므로, 켜진 채 방치되면 화면에 아무 표시 없이 배터리만 마릅니다. 현재 상태는 `pmset -g | grep SleepDisabled` 로 확인합니다.

내 기기에서 어떤지 보려면, 몇 초마다 타임스탬프를 찍어두고 뚜껑을 닫은 뒤 끊긴 구간을 보면 됩니다.

```bash
while true; do date +%H:%M:%S >> /tmp/hb.log; sleep 5; done
```

끊긴 구간이 곧 잠들어 있던 시간입니다. `SleepDisabled` 가 걸려 있으면 끊김이 없어야 합니다.
...
- **macOS 가 모든 예약 wake 를 보장하지는 않습니다.** 뚜껑·전원·sleep 깊이의 조합에 따라 다르고, M 시리즈에서 펌웨어 sleep 행 사례도 보고되어 있습니다. 대부분의 사이클은 정상 동작하며, 하나 빠져도 그 작업은 다음 사이클로 밀릴 뿐입니다.
