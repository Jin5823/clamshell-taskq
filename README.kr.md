...
뚜껑을 닫고 배터리로 있으면 `pmset` wake 가 맥을 완전히 깨우지 못합니다. **dark wake** 로만 올라오고, 약 45초 뒤 곧바로 다시 잠듭니다. runner 는 그 안에 충분히 끝나지만 `$COMMAND` 는 대개 그렇지 않습니다 — 도중에 동결됐다가 다음 wake 에 해동되므로, CPU 3분이면 되는 작업이 20분에 걸쳐 흩어지고, 동결 순간 붙들고 있던 연결은 해동 후 이미 끊겨 있습니다.

M4 Pro / macOS 26.5, 뚜껑 닫힘, 배터리에서 측정한 값입니다.

| | duty cycle |
|---|---|
| `caffeinate -i` 만 | **16%** — 45초 깨어 있고 4분 15초 자는 것의 반복 |
| `SleepDisabled` 설정 | **100%** — 36분 연속, sleep 0회 |

**어떤 파워 어시션으로도 해결되지 않습니다.** `caffeinate -i` 가 거는 것은 `PreventUserIdleSystemSleep` 인데, 여기서 일어나는 sleep 은 `Clamshell Sleep` 과 그 뒤의 `Maintenance Sleep` 입니다. 둘 다 idle sleep 이 아니므로 어시션은 정상적으로 유지되지만 애초에 해당 사항이 없습니다. `-s` 는 배터리에서 무시되고, 뚜껑 닫힘은 어떤 어시션도 넘지 못하는 하드웨어 경로입니다.

리드 클로즈를 넘어서는 것은 커널의 `SleepDisabled` 플래그뿐입니다. 그래서 `run.sh` 가 **작업이 있는 동안만** 이 플래그를 잡습니다.

```
runner 가 $COMMAND 를 띄움   →  pmset -a disablesleep 1
$COMMAND 아직 실행 중        →  그대로 유지
실행 중인 것 없음            →  pmset -a disablesleep 0
```

판단은 매 사이클마다 실제 프로세스 목록을 보고 다시 내립니다. 그래서 이 구조가 곧 정리 경로이기도 합니다 — 작업이 플래그를 풀지 못하고 죽어도 최대 한 사이클이면 해제됩니다. `SleepDisabled` 는 **재부팅을 넘어 유지**되므로, 켜진 채 방치되면 화면에 아무 표시 없이 배터리만 마릅니다.

즉 배터리 상태에서도 작업이 도는 동안에는 맥이 완전히 깨어 있습니다. 플래그가 남아 있는 것 같으면 `pmset -g | grep SleepDisabled` 로 확인하면 됩니다.
...
- **종료 코드만 보고 성공으로 단정하지 않는다.** headless 도구는 "실행이 중단됐다" 는 내용을 결과에 담은 채로 exit 0 으로 끝날 수 있습니다 — API 스트림이 중간에 끊기는 경우가 대표적입니다. 그걸 `✅` 로 찍으면 작업은 완료로 표시되고, 결과물은 없고, 재시도도 일어나지 않습니다. 어떻게 끝났는지가 아니라 **무엇을 반환했는지**를 확인하세요.
...
- **작업이 도는 동안 맥이 완전히 깨어 있고, 배터리에서는 그만큼 소모됩니다.** `$COMMAND` 가 실행되는 내내 `SleepDisabled` 가 걸립니다. 아직 배터리 하한 가드가 없으므로, 밤새 긴 작업을 배터리로 돌릴 생각이면 작업량을 감안하거나 전원을 연결해 두세요.
- **macOS 가 모든 예약 wake 를 보장하지는 않습니다.** 뚜껑·전원·sleep 깊이의 조합에 따라 다르고, M 시리즈에서 펌웨어 sleep 행 사례도 보고되어 있습니다. 여기서 측정한 실행들에서는 5분 RTC wake 가 하나도 빠지지 않았지만, 모든 환경에서 그러리라고 가정하지는 마세요.
